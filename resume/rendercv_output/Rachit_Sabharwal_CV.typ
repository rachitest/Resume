...
#emph[Doctor of Philosophy in Epidemiology]
  ],
  right-content: [
    #emph[Houston, TX]
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Used Deep Transfer Learning via PyTorch and Raytune to create a DNN to predict ADEs for DILI \(Drug-induced Liver Injury\)],[Created a Knowledge Graph with Neo4j and a Graph Neural Network using NetworkX and PyTorch to generate gene expression signature-likes for drugs],[Developed a framework for the tokenization of internal documents for ingestion into text-mining application],)
  ],
)
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Utilized Python, HTML, CSS, and Javascript in creating a multifeatured patent recommendation app to significantly improve scientists' workflow],[Developed and serviced ETL pipelines using Python and Apache Airflow for multiple datasets of varying sizes \(small, medium, and large\)],[Designed and maintained both relational and graph databases in PostgreSQL and Neo4j],)
  ],
)
...
#v(design-highlights-top-margin - design-text-leading)UTHealth School of Public Health, MS Thesis])
...
#one-col-entry(
  content: [#strong[Machine Learning Tooling:] Scikit-learn, TidyModels, Raytune, Optuna, Pytorch, Tensorflow, Huggingface, JAX]
)
#v(design-entries-vertical-space-between-entries)
#one-col-entry(
